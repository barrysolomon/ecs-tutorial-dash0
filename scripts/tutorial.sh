...
echo ""
echo -e "    ${C}┌──────────────────────────────────────────────────────────┐${NC}"
echo -e "    ${C}│${NC}  ECS Fargate Task  ${DIM}(both containers share network)${NC}      ${C}│${NC}"
echo -e "    ${C}│${NC}                                                          ${C}│${NC}"
echo -e "    ${C}│${NC}  ┌─────────────────────┐  ┌────────────────────────┐    ${C}│${NC}"
echo -e "    ${C}│${NC}  │ ${G}app${NC} (Node.js)       │  │ ${G}otel-collector${NC}          │    ${C}│${NC}"
echo -e "    ${C}│${NC}  │ Auto-instrumented   │─→│ resourcedetection      │──→ ${Y}Dash0${NC}"
echo -e "    ${C}│${NC}  │ + manual spans      │  │ batch, filter, retry   │    ${C}│${NC}"
echo -e "    ${C}│${NC}  │ port 3000           │  │ port 4317 (gRPC)       │    ${C}│${NC}"
echo -e "    ${C}│${NC}  └─────────────────────┘  └────────────────────────┘    ${C}│${NC}"
echo -e "    ${C}│${NC}       ↑                                                  ${C}│${NC}"
echo -e "    ${C}│${NC}  ALB :80 → :3000                                         ${C}│${NC}"
echo -e "    ${C}└──────────────────────────────────────────────────────────┘${NC}"
echo ""
explain "The collector config is a YAML pipeline:"
echo ""
echo -e "    ${DIM}receivers  → processors (in order)            → exporters${NC}"
echo -e "    ${DIM}  otlp       memory_limiter (always first)       otlp/dash0${NC}"
echo -e "    ${DIM}              → resourcedetection (ecs, ec2)${NC}"
echo -e "    ${DIM}              → filter/health (drop ALB noise)${NC}"
echo -e "    ${DIM}              → batch (always last)${NC}"
echo ""
